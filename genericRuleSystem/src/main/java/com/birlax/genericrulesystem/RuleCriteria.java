...
public class RuleCriteria implements Comparable<RuleCriteria> {
	...
	/**
	 * Rule can have multiple criteria or {@link RuleValue}, each criteria has
	 * weightage, Rule with higher overall weight is evaluate first. Overall rank
	 * for a rules is computed using the Map. <br>
	 * <br>
	 * <br>
	 * Putting absurd high values for rank may lead to failure of rule-system init
	 * process due to <b>Number Over/Under flow exceptions</b>.<br>
	 * <br>
	 * Also giving same ranks to all input criteria will not produce best results.
	 * Try to keep the values in same order as number of inputs. Say your
	 * rule-system has 12 input criteria try to give every input different rank and
	 * don't exceed the ranks more than 12. Even if ranks are provided with values
	 * higher than 12 rule-system will <b>continue to work and honor the ranking</b>
	 * until the Over/Under flow exceptions occur. <br>
	 * <br>
