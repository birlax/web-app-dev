...
public class StagingUtilDao {
  ...
  @Inject private StagingUtilMapper stagingUtilMapper;
   ...
   */
  @Inject private String temporaryDatabaseOrSchemaName;

  /**
   * Create a staging table against the schema of provided table and returns the name of create temp
   * table.<br>
   * <br>
   * Table will always be create in temp DATABASE(Microsoft SQL) or in temp schema in current
   * DATABASE(For PostgreSQL). Only for Postgres SQL, table will be always be created in same
   * database that's in current use, hence all databases should have above mentioned workaround.<br>
   * <br>
   * Note : Above is true even when fully qualified name from temp table is passed.<br>
   * Postgres also does not have concept for GTT (Global Temp Table) and I don't wan't to build one.
   * <br>
   * <br>
   * Note : When copying schema from any parent table, Child table's columns will be nullable, even
   * if columns in parent table was non-nullable. As temp tables are mostly used to staging this is
   * fine. But on bulk loading data to main(parent) table this may lead to failures. Other SQLs :
   * Microsoft SQL has different approachm there stating it-self will fail.
   */
  public String createTempStageTable(Set<String> columnNames, String fullyQualifiedParentTableName)
      throws IllegalArgumentException {
    if (columnNames == null
        || columnNames.isEmpty()
        || fullyQualifiedParentTableName == null
    ...
    log.debug(
        "Creating temp table : against master table : {} with columns : {}",
        fullyQualifiedParentTableName,
        columnNames);
    String tempTableName =
        temporaryDatabaseOrSchemaName
            + ".t"
            + Thread.currentThread().getId()
            + fullyQualifiedParentTableName.replaceAll("\\.", "_")
            + new Date().getTime();
    return createTempStageTableWithTableName(
        columnNames, fullyQualifiedParentTableName, tempTableName);
  }

  /** Helper method to {@link #createTempStageTable} */
  private String createTempStageTableWithTableName(
      Set<String> columnNames, String fullyQualifiedParentTableName, String tempTableName) {
    stagingUtilMapper.createTempStageTable(
        columnNames, fullyQualifiedParentTableName, tempTableName);
    return tempTableName;
  }

  /**
   * Transaction propagation is set to NOT_SUPPORTED so creation of staging table and inserting data
   * into staging table happens outside of the main transaction. Staging records into table is done
   * with COPY method. If that fails for any reason, it falls back to using SQL method. The fallback
   * is created as the SQL way is proven in production and this method is a very critical one.
   */
  // @Transactional(propagation = Propagation.NOT_SUPPORTED)
  public String dummyStageRecords(
      Set<String> createTableColName,
      Set<String> insertColName,
      Set<String> jsobInsertColName,
    ...
    if (insertColName == null
        || insertColName.isEmpty()) { // still, create the table as other queries may depend on
    ...
    try {
      ...
      log.error(
          "Error creating temp/staging table from table : " + fullyQualifiedParentTableName, e);
    }

    log.info(
        "Temp/Staging table : {} create. Start staging records size :{}",
        tempTableName,
        dataRecords.size());

    try {
      log.debug(
          "Loading records into a staging table : {} using COPY for table : {}",
          tempTableName,
          fullyQualifiedParentTableName);

      insertRecordsIntoStagingTableUsingCopy(
          insertColName,
          jsobInsertColName,
          dataRecords,
          tempTableName,
      ...
      // log.error("Error in loading records into staging table through COPY for " +
      // fullyQualifiedParentTableName
      //       + " staging table : " + tempTableName, e);
    }

    try {
      log.debug("Retrying the SQL way.");
      tempTableName =
          retryDummyStageRecordsThroughSQL(
              insertColName, jsobInsertColName, tempTableName, dataRecords);
    } catch (Exception e) {
      log.error(
          "Error in loading records into staging table through SQL  "
              + fullyQualifiedParentTableName
              + " staging table : "
              + tempTableName,
          e);
  ...
  private String retryDummyStageRecordsThroughSQL(
      Set<String> insertColName,
      Set<String> jsobInsertColName,
      String tempTableName,
      List<Map<String, Object>> dataRecords) {
    ...
    // String tempTableName = null;
    try {
      // log.debug("Loading records into a staging table using SQL for {}",
      // fullyQualifiedParentTableName);
      // tempTableName = createTempStageTable(createTableColName, fullyQualifiedParentTableName);
      ...
      log.error(
          "Error in loading records into staging table through SQL for staging table : "
              + tempTableName,
          e);
      throw e;
    }
  ...
  private void insertRecordsIntoStagingTable(
      Set<String> insertColName,
      Set<String> jsobInsertColName,
      List<Map<String, Object>> dataRecords,
      String tempTableName) {

    // if (dataRecords == null || dataRecords.isEmpty()) {// still, create the table as other
    // queries may depend on
    // presence of the table
    // log.info("Skipping staging table persistence as record set is empty for " + tempTableName);
    // return;
    // }

    log.debug(
        "Starting staging records to sandbox/temp table in bulk [ "
            + tempTableName
            + " ] Records to be staged [ "
            + dataRecords.size()
            + " ].");
    ...
    while (fromIndex < dataRecords.size()) {
      ...
      stagingUtilMapper.dummyStageRecordsInBulk(
          tempTableName, insertColName, jsobInsertColName, dataRecordsForABatch);
      fromIndex = toIndex;
    }
    log.debug(
        "Completed staging records to sandbox/temp table [ "
            + tempTableName
            + " ] Records staged [ "
            + dataRecords.size()
            + " ].");
  ...
  private void insertRecordsIntoStagingTableInBatchesUsingSqls(
      Set<String> insertColName,
      Set<String> jsobInsertColName,
      List<Map<String, Object>> dataRecords,
      String tempTableName,
    ...
    if (dataRecords == null
        || dataRecords.isEmpty()) { // still, create the table as other queries may depend on
      // presence of the table
      log.debug("Skipping staging table persistence as record set is empty for " + tempTableName);
      return;
    }
    log.debug(
        "Starting staging records to sandbox/temp table in bulk in multiple batches [ "
            + tempTableName
            + " ] Records to be staged [ "
            + dataRecords.size()
            + " ].");
    ...
    List<String> jsobColumnNames =
        jsobInsertColName != null ? new ArrayList<>(jsobInsertColName) : Collections.emptyList();

    List<Boolean> stringTypeColumns =
        databaseMetadataService.determineStringTypeColumns(
            fullyQualifiedParentTableName, insertColName, false);
    ...
    while (fromIndex < dataRecords.size()) {
      ...
      List<Map<String, Object>> dataRecordsForABatch = dataRecords.subList(fromIndex, toIndex);
      String insertSql =
          prepareInsertSqlForBatchOfRecords(
              insertColumnNames,
              jsobColumnNames,
              stringTypeColumns,
              insertSqlBase,
    ...
    log.debug("SQL:" + sqls.get(0));
    try {
      stagingUtilMapper.dummyStageRecordsUsingSqls(sqls, tempTableName);
    } catch (Exception e) {
      log.error("Error in staging sql insert", e);
      throw e;
    }
    log.debug(
        "Completed staging records to sandbox/temp table  [ "
  ...
  /**
   * This uses COPY method of postgres to write data to staging tables. This has proved to be the
   * fastest way to send in bulk data from Java to DB. Other methods are left in the class for
   * reference for now till this stabilizes.
   */
  private void insertRecordsIntoStagingTableUsingCopy(
      Set<String> insertColName,
      Set<String> jsobInsertColName,
      List<Map<String, Object>> dataRecords,
      String tempTableName,
    ...
    log.debug(
        "Starting staging records to sandbox/temp table through csv copy [ "
            + tempTableName
            + " ] Records to be staged [ "
            + dataRecords.size()
            + " ].");
    ...
    // Connection and BaseConnection have to be created separately, otherwise releaseConnection will
    // not work
    ...
    try {
      ...
      // log.error("Error in creating CopyManager to copy records.", e);
      ...
      if (connection != null) {
        try {
          ...
          // Nothing to do
          log.error("Error in releasing connection taken for COPY. Nothing to do.", e);
        }
      }
    }
  ...
  private void copyRecordsToTableInBatches(
      List<Map<String, Object>> dataRecords,
      String tempTableName,
    ...
    // Records are copied to table in batches to keep the CSV records stringbuffer memory to a
    // reasonable size
    int toIndex = 0, fromIndex = 0;
    int batchSize =
        20000; // 20K records with 1K record size will take 20MB of memory for a stringbuilder
    while (fromIndex < dataRecords.size()) {
      toIndex = fromIndex + batchSize;
      if (toIndex > dataRecords.size()) {
        toIndex = dataRecords.size();
  ...
  private void copyRecordsToTable(
      List<Map<String, Object>> dataRecords,
      List<String> insertColumnNames,
      List<String> jsobColumnNames,
      List<Boolean> stringTypeColumns,
    ...
    try {
      String content =
          writeRecordsToMemory(dataRecords, insertColumnNames, jsobColumnNames, stringTypeColumns);
      StringReader strReader = new StringReader(content);
      copyManager.copyIn(sql, strReader);
    } catch (Exception e) {
      log.error("Error in creating file and copying records. SQL:" + sql, e);
      throw new RuntimeException(e);
    }
  }

  private String writeRecordsToMemory(
      List<Map<String, Object>> dataRecords,
      List<String> insertColumnNames,
      List<String> jsobColumnNames,
      List<Boolean> stringTypeColumns)
    ...
    StringBuilder contentBuilder =
        new StringBuilder(dataRecords.size() * insertColumnNames.size() * 10);
    for (Map<String, Object> record : dataRecords) {
      contentBuilder.append(
          convertRecordToCsv(insertColumnNames, jsobColumnNames, stringTypeColumns, record));
  ...
  /**
   * CSV record is prepared. String columns are enclosed with double quotes, comma is the separator,
   * an empty unquoted string is considerd as null.
   */
  private String convertRecordToCsv(
      List<String> insertColumnNames,
      List<String> jsobColumnNames,
      List<Boolean> stringTypeColumns,
  ...
  private String prepareInsertSqlForBatchOfRecords(
      List<String> insertColumnNames,
      List<String> jsobColumnNames,
      List<Boolean> stringTypeColumns,
      String insertSqlBase,
      List<Map<String, Object>> dataRecordsForABatch) {
  ...
  private String getBaseInsertSql(
      String tempTableName, List<String> insertColumnNames, List<String> jsobColumnNames) {
    ...
    if (jsobColumnNames != null && !jsobColumnNames.isEmpty()) {
      String delimitedInsertJsonbColumns =
          jsobColumnNames.stream().collect(Collectors.joining(","));
  ...
  /**
   * @param tempTableName Drops that give table only if it exists and is in sandbox schema
   */
  public void dropTempTable(String tempTableName) {
    if (tempTableName == null
        || tempTableName.isEmpty()
        || !tempTableName.contains(temporaryDatabaseOrSchemaName)) {
      throw new IllegalArgumentException(
          "Attempt to drop master table. Or table is invalid/none-existent..");
    }
    try {
      stagingUtilMapper.dropTempTable(tempTableName);
    } catch (Exception e) {
      log.warn("Unable to drop staging table : {} ", tempTableName, e);
  ...
  /**
   * Note : COPY Manager from PG need dbh more like perl DBO, and I am not yet able to figure out
   ...
   * log.error("IO Exception while creating StringReader...", exception); throw new
   * IllegalArgumentException("IO Error can't proceed forward..", exception); } } catch (SQLException exception) {
   * log.error("Error staging record.", exception); throw new
